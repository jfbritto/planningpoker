...
<div class="card room-header">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <div style="flex: 1; min-width: 200px;">
            <h1 style="margin-bottom: 8px; font-size: 18px;">{{ $room->name }}</h1>
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                <span style="font-size: 11px; color: #999; text-transform: uppercase; letter-spacing: 0.5px;">Código:</span>
                <button id="room-code-btn" class="room-code-link" data-code="{{ $room->code }}" title="Clique para copiar URL completa">
                    <span id="room-code-text">{{ $room->code }}</span>
...
@if(!$participant)
    <div class="card" style="background: #fff3cd; border-left: 3px solid #ffc107;">
        ...
        <form method="POST" action="{{ route('rooms.join', $room->code) }}" id="join-form">
            @csrf
            <div class="form-group">
                <label for="name">Seu Nome</label>
                <input type="text" id="name" name="name" required placeholder="Digite seu nome" value="{{ $savedName ?? '' }}">
            </div>
    ...
    <div class="card" id="voting-area">
        @if($activeStory)
            <div id="story-info" style="margin-bottom: 12px;">
                <h2 id="story-title" style="margin-bottom: 0;">{{ $activeStory->title }}</h2>
            </div>
            ...
            @if(!$activeStory->is_revealed)
                ...
                <div id="votes-status" style="margin-top: 16px;">
                        ...
                        @foreach($participants as $p)
                            @if(true)
                                @php
                                    $vote = $p->votes->first();
                                @endphp
                                <div class="participant-card emoji-target" data-participant-id="{{ $p->id }}" data-participant-name="{{ $p->name }}">
                                    ...
                                    @if($participant && $participant->id !== $p->id)
                    ...
                    <p style="margin-top: 10px; color: #666; font-size: 12px;">
                        <span id="votes-count">{{ $participants->filter(function($p) { return $p->votes->isNotEmpty(); })->count() }}</span> de <span id="total-participants">{{ $participants->count() }}</span> votaram
                    </p>
                </div>
                ...
                <div id="results-container">
                    <div class="results">
                        <div class="result-card">
                            <div class="result-value" id="average-vote">{{ $activeStory->average_vote ?? 'N/A' }}</div>
                            <div class="result-label">Média</div>
                        </div>
                        <div class="result-card">
                            <div class="result-value" id="total-votes-count">{{ $activeStory->votes->count() }}</div>
                        ...
                        @foreach($participants as $p)
                            @if(true)
                                ...
                                <div class="participant-card emoji-target" data-participant-id="{{ $p->id }}" data-participant-name="{{ $p->name }}">
                                    <div class="participant-name">{{ $p->name }}</div>
                                    <div class="participant-vote">
                                        Voto: <strong style="color: #667eea; font-size: 16px;">{{ $vote ? $vote->value : 'Não votou' }}</strong>
                                    </div>
                                    @if($participant && $participant->id !== $p->id)
                                        <div class="emoji-picker" style="display: none;">
                                            <div class="emoji-options">
...
<script>
    ...
    (function() {
        ...
        const savedName = @json($savedName ?? null);
    ...
    const activeStoryId = {{ $activeStory ? $activeStory->id : 'null' }};
    let isRevealed = {{ $activeStory && $activeStory->is_revealed ? 'true' : 'false' }};
    const isCreator = {{ isset($isCreator) && $isCreator ? 'true' : 'false' }};
    const participantId = {{ $participant ? $participant->id : 'null' }};
    let lastUnrevealedCount = {{ $room->stories()->where('is_revealed', false)->count() }};
